...
public:
    ...
    int num_of_digits(int n)
    {
        ...
        int res = 0;
        for(int width = 0; width <= s.size(); width++) 
        {
            res += dfs(0, width, 0, width < s.size());
        }
        return res;
    }
    ...
    int dfs(int idx, int width, int cnt = 0, bool smaller = false)
    {
        if(idx == width) 
            return cnt;
        if(memo[idx][width][cnt][smaller] >= 0) 
            return memo[idx][width][cnt][smaller];
        int res = 0;
        if(smaller)
        {
            int min_digit = idx == 0 ? 1 : 0;
            for(int digit = min_digit; digit < 10; digit++)
            {
                res += dfs(idx + 1, width, cnt + (digit == d), true);
            }
        }
        else
        {
